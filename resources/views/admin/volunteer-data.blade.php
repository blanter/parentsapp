...
@section('content')
    ...
    <div
        ...
        <div class="adm-card" style="padding: 24px;">
            <div style="display: flex; align-items: center; gap: 15px;">
                <div
                    style="width: 54px; height: 54px; background: rgba(108, 136, 224, 0.1); border-radius: 16px; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
                    <i data-lucide="calendar-check" style="width: 26px; color: var(--db-purple);"></i>
                </div>
                <div style="flex: 1; min-width: 0;">
                    <div
                        style="font-size: 11px; font-weight: 700; color: #9CA3AF; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 6px;">
                        This Week</div>
                    <div style="font-size: 28px; font-weight: 900; color: var(--db-text-dark); line-height: 1;">
                        {{ $weeklyCompletions }}</div>
                </div>
            </div>
        </div>
        ...
        <div class="adm-card" style="padding: 24px;">
            <div style="display: flex; align-items: center; gap: 15px;">
                <div
                    style="width: 54px; height: 54px; background: rgba(255, 107, 74, 0.1); border-radius: 16px; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
        ...
        <div class="adm-card" style="padding: 24px;">
            <div style="display: flex; align-items: center; gap: 15px;">
                <div
                    style="width: 54px; height: 54px; background: rgba(255, 214, 75, 0.1); border-radius: 16px; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
        ...
        <div class="adm-card" style="padding: 24px;">
            <div style="display: flex; align-items: center; gap: 15px;">
                <div
                    style="width: 54px; height: 54px; background: rgba(34, 197, 94, 0.1); border-radius: 16px; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
    ...
    <div class="adm-card">
        <div class="adm-card-header">
            <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 15px;">
                <h3 style="font-weight: 800; font-size: 18px; color: var(--db-text-dark); margin: 0;">Parent Activity
                    Overview</h3>
                <div
                    style="background: rgba(108, 136, 224, 0.1); color: var(--db-purple); padding: 6px 16px; border-radius: 12px; font-weight: 800; font-size: 13px; display: inline-flex; align-items: center; gap: 8px;">
                            ...
                            <tr>
                                <td style="padding-left: 24px;">
                                    <div style="display: flex; align-items: center; gap: 12px;">
                                        <div
                                            style="width: 42px; height: 42px; background: var(--db-purple); border-radius: 50%; display: flex; align-items: center; justify-content: center; overflow: hidden;">
                                            @if($stat['user']->avatar)
                                                <img src="{{ asset('avatars/' . $stat['user']->avatar) }}"
                                        ...
                                        <div>
                                            <div style="font-weight: 800; color: var(--db-text-dark); font-size: 14px;">
                                                {{ $stat['user']->name }}</div>
                                ...
                                <td style="text-align: center;">
                                    <div
                                        style="display: inline-flex; align-items: center; gap: 6px; background: rgba(108, 136, 224, 0.1); color: var(--db-purple); padding: 6px 14px; border-radius: 10px; font-weight: 800; font-size: 14px;">
                                ...
                                <td style="text-align: center;">
                                    <span
                                        style="font-weight: 700; font-size: 14px; color: var(--db-text-dark);">{{ $stat['total_completions'] }}</span>
                                </td>
                                <td style="text-align: center;">
                                    @if($stat['current_streak'] > 0)
                                        <div
                                            style="display: inline-flex; align-items: center; gap: 6px; background: rgba(255, 107, 74, 0.1); color: var(--db-accent); padding: 6px 14px; border-radius: 10px; font-weight: 800; font-size: 13px;">
                                ...
                                <td style="padding-right: 24px; text-align: right;">
                                    <button class="btn-warning"
                                        onclick="viewUserDetail('{{ $stat['user']->id }}', '{{ $stat['user']->name }}')"
                        ...
                        @empty
                            <tr>
                                <td colspan="5" style="text-align: center; padding: 60px 20px;">
                                    <i data-lucide="inbox"
                                        style="width: 48px; height: 48px; color: #9CA3AF; margin-bottom: 15px;"></i>
    ...
    <!-- Modal User Detail -->
    <div id="userDetailModal" class="ct-modal-overlay">
        <div class="ct-modal-card" style="max-width: 800px;">
            <div class="ct-modal-header">
                <div class="ct-modal-title-group">
                    ...
                    <div>
                        <h3 class="ct-modal-title">Volunteer Mission Detail</h3>
                        <p class="ct-modal-subtitle" id="modalUserName">Parent Data
                        </p>
                    </div>
                </div>
                <button class="ct-modal-close" onclick="closeUserDetailModal()">
                    <i data-lucide="x"></i>
                </button>
            </div>
            <div class="ct-modal-body" id="userMissionList"
    ...
    <script>
        ...
        function viewUserDetail(userId, userName) {
            ...
            $('#modalUserName').text(`${userName} • This Week Activity`);

            const missionList = $('#userMissionList');
            missionList.html('');
            ...
            if (userStat.weekly_completions === 0) {
                missionList.html(`
                        <div class="ct-premium-empty-wrapper">
                            <div class="ct-premium-empty-content">
                                <i data-lucide="calendar-x" class="ct-premium-empty-icon"></i>
                                <p class="ct-premium-empty-text">No missions completed this week.</p>
                            </div>
                        </div>
                    `);
            } else {
                missionsData.forEach(mission => {
                    ...
                    if (completions.length > 0) {
                        ...
                        const itemHtml = `
                                <div class="adm-gn-progress-item shadow-sm" style="margin-bottom: 20px;">
                            ...
                            `;
                        missionList.append(itemHtml);
                    }
                });
            }

            $('#userDetailModal').fadeIn(300).css('display', 'flex');
            lucide.createIcons();
        }

        function closeUserDetailModal() {
            $('#userDetailModal').fadeOut(300);
